...
describe("launcher", () => {
  it("renders the tool catalog without a crash surface", async () => {
    ...
    await expect(
      $$('[data-testid="application-launcher-catalog"] [aria-label="Archives explorer"]')
    ).toBeElementsArrayOfSize(1);
    await expect(
      $$('[data-testid="application-launcher-catalog"] [aria-label="Spawn editor"]')
    ).toBeElementsArrayOfSize(1);
  ...
  it("opens a tool and returns to the launcher", async () => {
    await $('[data-testid="application-launcher-catalog"]').waitForExist({ timeout: 10_000 });
    await $('[data-testid="application-launcher-catalog"] [aria-label="Archives explorer"]').click();

    const toolbar = $('[data-testid="editor-toolbar"]');

    await expect(toolbar).toHaveText(expect.stringContaining("Archives explorer"));
    await expect($('[data-testid="application-launcher-catalog"]')).not.toBeExisting();
    await expect($("*=This tool stopped rendering")).not.toBeExisting();
