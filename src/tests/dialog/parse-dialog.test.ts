...
describe("parse-dialog", () => {
  ...
  beforeAll(() => {
    swept = box.run("parse-dialog", ["--path", GAMEDATA]);
    prefixed = box.run("parse-dialog", ["--path", GAMEDATA, "--prefix", "configs\\gameplay"]);
    detailed = box.run("parse-dialog", ["--path", GAMEDATA, "--verbose"]);
    report = box.run("parse-dialog", ["--path", GAMEDATA, "--report", box.at("report.json")]);

    // Findings are chatter, so a silent run says nothing at all until the verdict is a failure.
    silent = box.run("parse-dialog", ["--path", GAMEDATA, "--silent"]);
    ...
    elsewhere = box.run("parse-dialog", ["--path", GAMEDATA, "--prefix", "meshes"], { expectExit: 1 });
  });
  ...
  it("should fail under strict when a file is off schema", () => {
    expect(box.run("parse-dialog", ["--path", GAMEDATA, "--strict"], { expectExit: 3 })).toMatchSnapshot();
  });
  ...
  it("should report the failure even when silenced", () => {
    expect(box.run("parse-dialog", ["--path", GAMEDATA, "--silent", "--strict"], { expectExit: 3 })).toMatchSnapshot();
  });
