...
/**
 * The six-field installation form the engine reads, as `list-assets` stages it: `$game_data$`
 ...
 */
describe("parse-dialog over an installation", () => {
  ...
  beforeAll(() => {
    ...
    installation = box.run("parse-dialog", ["--path", box.at("install"), "--verbose"]);
    loose = box.run("parse-dialog", ["--path", gamedata(), "--verbose"]);
  });
